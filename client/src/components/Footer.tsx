import { motion } from "framer-motion";

export default function Footer() {
  return (
    <footer
      id="contact"
      className="bg-black text-white pt-32 pb-12 overflow-hidden scroll-mt-28"
    >
      <div className="container mx-auto px-6">
        <div className="border-b border-white/10 pb-20 mb-20">
          <h2 className="text-[10vw] leading-[0.85] font-bold tracking-tighter mb-10">
            LET'S WORK <br />
            <span className="text-white/30">TOGETHER</span>
          </h2>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10">
            <a
              href="mailto:contact@theaussiedevs.com"
              className="text-2xl md:text-4xl hover:underline underline-offset-8 decoration-1"
            >
              contact@theaussiedevs.com
            </a>

            <div className="flex gap-8">
              {[
                { name: "Instagram", url: "https://www.instagram.com/ozedevs/" },
                { name: "Twitter", url: "https://x.com/Ozedevs" },
                {
                  name: "LinkedIn",
                  url: "https://www.linkedin.com/in/mateen-qazi-b55504400/",
                },
                { name: "Github", url: "https://github.com/Streban" },
              ].map((social) => (
                <a
                  key={social.name}
                  href={social.url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-sm font-medium tracking-widest uppercase hover:text-accent transition-colors"
                >
                  {social.name}
                </a>
              ))}
            </div>
          </div>
        </div>

        <div className="flex flex-col md:flex-row justify-between text-white/30 text-xs uppercase tracking-wider">
          <p>© 2026 Portfolio. All rights reserved.</p>
          <p>Designed & Built with ❤️</p>
        </div>
      </div>
    </footer>
  );
}
